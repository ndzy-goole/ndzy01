...
import { resetBreadcrumb, setSelectKeys, setOpenKeys, MyStore } from '@/redux';

import { ActionFunctionAny } from 'redux-actions';
import { Action } from 'redux';
import { historyHash as history } from '@_u/index';

import { useMount } from 'ahooks';

// utils
import { getOpenKeysG } from '../utils/root';
...
export default connect(mapStateToProps, {
  ...
  const handleCloseTab = (path: string, e: MouseEvent) => {
    ...
    if (props.selectedKeys.includes(path)) {
      ...
      // 侧边栏收缩时不设置openKey
      if (!props.collapsed) {
        props.setOpenKeys && props.setOpenKeys(getOpenKeysG(newPath));
      }
